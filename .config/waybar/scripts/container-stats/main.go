...
func getContainers() []Container {
	cmd := exec.Command("/usr/bin/podman", "ps", "-a", "--format", "{{.Names}}|{{.Status}}|{{.ID}}")
	...
	for _, container := range containers {
		if container.Status == "Running" {
			...
			go func(container Container) {
				defer wg.Done()
				cmd := exec.Command("podman", "stats", container.ID, "--no-stream", "--format", "{{.MemUsage}}")
				output, err := cmd.Output()
				if err != nil {
					return
				}
...
func main() {
	...
	output := WaybarOutput{
		...
		Tooltip: tooltip,
		Class:   "podman",
		Alt:     fmt.Sprintf("Podman (%d/%d)", len(runningContainers), len(allContainers)),
	}

	if len(allContainers) == 0 {
		output = WaybarOutput{
			Text:    "󰡨 N/A",
			Tooltip: "No Podman containers found",
			Class:   "podman-none",
			Alt:     "Podman (0/0)",
		}
	}
